...
class CommitteeApprovalComponent(ServiceComponent):
    ...
    def _validate_identifier_changes(self, identity, data, record):
        ...
        incoming = (data.get("metadata") or {}).get("identifiers", [])
        stored = (record.get("metadata") or {}).get("identifiers", [])

        # Index stored apprn values for comparison.
        stored_apprn = {
            i["identifier"] for i in stored if i.get("scheme") == "apprn"
        }
        incoming_apprn = {
            i["identifier"] for i in incoming if i.get("scheme") == "apprn"
        }
        ...
        if ep_prefixes:
            errors = []
            for index, ident in enumerate(incoming):
    ...
    def _regenerate_apprn_identifier(self, record, data):
        ...
        """
        ea = ((record.parent.get("permission_flags") if record.parent else None) or {}).get("ep_approval") or {}
