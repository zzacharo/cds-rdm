...
class EPApprovalSubmitAction(actions.CreateAndSubmitAction):
    ...
    def execute(self, identity: Identity, uow: UnitOfWork) -> None:
        ...
        from cds_rdm.generators import EPWorkflowCommunityManager
        ...
        if ((topic.parent.get("permission_flags") or {}).get("ep_approval") or {}).get("reportnumber"):
            raise ValidationError(
...
class EPApprovalAcceptAction(actions.AcceptAction):
    ...
    def _generate_report_number(self, pattern: str) -> str:
        ...
        max_seq = max(
            (int(p.pid_value[len(prefix):]) for p in existing if p.pid_value[len(prefix):].isdigit()),
            default=0,
        )
